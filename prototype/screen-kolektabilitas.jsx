/* ============================================================
   SCREEN: Kolektabilitas — postur detail + tabel nasabah
   ============================================================ */
...
// urutan + warna akad (shared chroma/lightness, varied hue) untuk komposisi
const AKAD_LIST = ["Murabahah", "Musyarakah", "Ijarah", "Musyarakah Mutanaqisah", "Istishna"];
const AKAD_COLOR = {
...
function ScreenKolektabilitas({ go }) {
  const [fKol, setFKol] = useState("all");
  const [fPet, setFPet] = useState("all");
  const [fAkad, setFAkad] = useState("all");
  ...
  return (
    <div className="content">
        ...
        {[1,2,3,4,5].map(k => {
          ...
          return (
            <button key={k} onClick={() => setFKol(active ? "all" : String(k))}
              className="card card-pad" style={{ textAlign: "left", cursor: "pointer",
          ...
          {akadComp.map(a => {
            ...
            return (
              <button key={a.akad} onClick={() => setFAkad(active ? "all" : a.akad)}
                style={{ textAlign: "left", background: active ? "var(--surface-2)" : "transparent", cursor: "pointer",
      ...
      <div className="card fade-up" style={{ overflow: "hidden" }}>
        <div className="between" style={{ padding: "14px 16px", borderBottom: "1px solid var(--line)", flexWrap: "wrap", gap: 12 }}>
          <div className="center gap-3" style={{ flexWrap: "wrap" }}>
            ...
            <select className="input" style={{ width: "auto" }} value={fAkad} onChange={e => setFAkad(e.target.value)}>
      ...
      {sel && <NasabahDrawer n={sel} onClose={() => setSel(null)} go={go} />}
    </div>
  );
}

function NasabahDrawer({ n, onClose, go }) {
  const p = petugasById(n.petugas);
...
window.ScreenKolektabilitas = ScreenKolektabilitas;
window.NasabahDrawer = NasabahDrawer;
