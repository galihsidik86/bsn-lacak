/* ============================================================
   SCREEN: Blast SMS / WhatsApp — belum & sudah jatuh tempo
   ============================================================ */

const SEGMENTS = [
  { key: "h3",       label: "Belum Jatuh Tempo (H-3)", desc: "Reminder dini untuk nasabah lancar", icon: "clock",
    c: "var(--col-dpk)", soft: "var(--col-dpk-soft)", list: () => SEGMEN.h3, tpl: "belum" },
  { key: "hari_ini", label: "Jatuh Tempo Hari Ini",    desc: "Tagihan jatuh tempo per hari ini", icon: "bell",
    c: "var(--accent)", soft: "var(--accent-soft)", list: () => SEGMEN.hari_ini, tpl: "hari_ini" },
  { key: "lewat",    label: "Lewat Jatuh Tempo",       desc: "Sudah menunggak, perlu penagihan", icon: "alert",
    c: "var(--col-macet)", soft: "var(--col-macet-soft)", list: () => SEGMEN.lewat, tpl: "lewat" },
];

function ScreenBlast() {
  const [segKey, setSegKey] = useState("hari_ini");
  const [kanal, setKanal] = useState("wa");
  const seg = SEGMENTS.find(s => s.key === segKey);
  const recipients = seg.list();
  const [tpl, setTpl] = useState(TEMPLATES[seg.tpl]);
  const [excluded, setExcluded] = useState(new Set());
  const [sending, setSending] = useState(null); // null | 'confirm' | 'progress' | 'done'

  useEffect(() => { setTpl(TEMPLATES[seg.tpl]); setExcluded(new Set()); }, [segKey]);
          ...
          return (
            <button key={s.key} onClick={() => setSegKey(s.key)} className="card card-pad fade-up"
              ...
              <div className="between">
                ...
                <div className="num" style={{ fontSize: 26, fontWeight: 800, letterSpacing: "-0.03em" }}>{s.list().length}</div>
              </div>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "1.3fr 1fr" }}>
        ...
        <div className="card card-pad fade-up">
          <div className="between" style={{ marginBottom: 16 }}>
            ...
            <div className="seg">
              <button className={kanal === "wa" ? "on" : ""} onClick={() => setKanal("wa")}><span className="center gap-2"><Ic.wa size={15} style={{ color: kanal==="wa"?"var(--wa)":"inherit" }} />WhatsApp</span></button>
              <button className={kanal === "sms" ? "on" : ""} onClick={() => setKanal("sms")}><span className="center gap-2"><Ic.sms size={15} style={{ color: kanal==="sms"?"var(--sms)":"inherit" }} />SMS</span></button>
            </div>
          </div>
          ...
          <div className="center gap-2" style={{ marginTop: 10, flexWrap: "wrap" }}>
            ...
            <span style={{ marginLeft: "auto", fontSize: 11.5, color: "var(--ink-4)", fontWeight: 600 }}>{tpl.length} karakter {kanal === "sms" && `· ${Math.ceil(tpl.length/160)} SMS`}</span>
          </div>
          ...
          <div className="between" style={{ marginTop: 18 }}>
            <div className="muted" style={{ fontSize: 12.5, fontWeight: 600 }}>
              Estimasi biaya: <span className="num" style={{ color: "var(--ink)", fontWeight: 700 }}>{kanal === "sms" ? RP(active.length * 350) : "Gratis (WA Business)"}</span>
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div className="card card-pad fade-up">
            ...
            <PhonePreview kanal={kanal} text={preview} nama={sample?.nama} />
          </div>
            ...
            <div style={{ padding: "0 6px 6px" }}>
              {BLAST_HISTORY.map(b => (
                <div key={b.id} className="between" style={{ padding: "10px 12px", borderTop: "1px solid var(--line)" }}>
                  <div className="center gap-3">
                    <div className="stat-ic" style={{ width: 34, height: 34, background: b.kanal === "wa" ? "var(--accent-soft)" : "oklch(0.93 0.04 245)", color: b.kanal === "wa" ? "var(--wa)" : "var(--sms)" }}>
                      {b.kanal === "wa" ? <Ic.wa size={16} /> : <Ic.sms size={16} />}
                    </div>
                    <div>
                  ...
                  <div style={{ textAlign: "right" }}>
                    {b.status === "terjadwal"
                      ...
                      : <div className="num" style={{ fontSize: 12, fontWeight: 700 }}>{b.terkirim}/{b.target} <span className="muted" style={{ fontWeight: 600 }}>terkirim</span></div>}
                  </div>
                </div>
              ))}
            </div>
...
function PhonePreview({ kanal, text, nama }) {
  return (
    <div style={{ background: kanal === "wa" ? "oklch(0.93 0.02 150)" : "var(--surface-2)", borderRadius: 16, padding: 16, minHeight: 180,
      backgroundImage: kanal === "wa" ? "radial-gradient(oklch(0.85 0.03 150 / 0.5) 1px, transparent 1px)" : "none", backgroundSize: "16px 16px" }}>
      ...
      <div style={{ background: "var(--surface)", borderRadius: "4px 14px 14px 14px", padding: "11px 13px", fontSize: 12.5, lineHeight: 1.55, boxShadow: "var(--sh-1)", color: "var(--ink)" }}>
        {text}
...
function SendFlow({ seg, kanal, count, stage, setStage }) {
  ...
  return (
    <Modal onClose={() => stage !== "progress" && setStage(null)} max={440}>
        ...
        <>
          ...
          <div className="modal-body">
            ...
            <div className="card card-pad" style={{ background: "var(--surface-2)", boxShadow: "none" }}>
              <div className="between" style={{ fontSize: 13 }}><span className="muted">Kanal</span><span style={{ fontWeight: 700 }}>{kanal === "wa" ? "WhatsApp Business" : "SMS Gateway"}</span></div>
              <div className="between" style={{ fontSize: 13, marginTop: 8 }}><span className="muted">Penerima</span><span className="num" style={{ fontWeight: 700 }}>{count} nasabah</span></div>
              <div className="between" style={{ fontSize: 13, marginTop: 8 }}><span className="muted">Estimasi biaya</span><span className="num" style={{ fontWeight: 700 }}>{kanal === "sms" ? RP(count * 350) : "Gratis"}</span></div>
            </div>
          </div>
      ...
      {stage === "done" && (
        <div className="modal-body" style={{ textAlign: "center", padding: "36px 28px" }}>
          <div style={{ width: 64, height: 64, borderRadius: 99, background: "var(--accent-soft)", color: "var(--accent)", display: "grid", placeItems: "center", margin: "0 auto 18px" }}>
            <Ic.check size={32} />
          </div>
          <div style={{ fontWeight: 800, fontSize: 17 }}>Blast Terkirim!</div>
          <div className="muted" style={{ fontSize: 13.5, marginTop: 6, lineHeight: 1.5 }}>{count} pesan {kanal === "wa" ? "WhatsApp" : "SMS"} berhasil dikirim ke nasabah segmen {seg.label}.</div>
...
window.ScreenBlast = ScreenBlast;
