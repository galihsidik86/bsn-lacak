/* ============================================================
   SCREEN: Distribusi Nasabah Binaan ke Petugas Lapangan
   ============================================================ */

function ScreenDistribusi() {
  // local assignment state
  const [assign, setAssign] = useState(() => {
    const m = {}; NASABAH.forEach(n => m[n.id] = n.petugas); return m;
  });
  const [moving, setMoving] = useState(null); // nasabah being reassigned
  const [toast, setToast] = useState(null);

  const loadOf = (pid) => NASABAH.filter(n => assign[n.id] === pid);
  const CAP = 18; // kapasitas ideal per petugas

  const doAssign = (nid, pid) => {
    setAssign(a => ({ ...a, [nid]: pid }));
    const n = nasabahById(nid), p = petugasById(pid);
    setToast(`${n.nama} dipindahkan ke ${p.nama}`);
    setTimeout(() => setToast(null), 2600);
    setMoving(null);
  };

  const autoBalance = () => {
    // round-robin redistribute keeping wilayah-ish: simple even split
    const m = {};
    const sorted = [...NASABAH];
    sorted.forEach((n, i) => { m[n.id] = PETUGAS[i % PETUGAS.length].id; });
    setAssign(m);
    ...
    <div className="content">
      ...
      <div className="grid gap-4 fade-up" style={{ gridTemplateColumns: "repeat(3, 1fr)", marginBottom: 22 }}>
        {PETUGAS.map(p => {
          ...
          const segs = [1,2,3,4,5].map(k => ({ label: KOL[k].label, value: load.filter(n => n.kol === k).length, color: `var(${KOL[k].c.replace("var(","").replace(")","")})` }));
          ...
          return (
            <div key={p.id} className="card card-pad">
              ...
              <div className="between" style={{ marginTop: 16, marginBottom: 7 }}>
                <span className="center gap-2"><span className="num" style={{ fontSize: 22, fontWeight: 800 }}>{load.length}</span><span className="muted" style={{ fontSize: 12, fontWeight: 600 }}>nasabah</span></span>
                <span className={over ? "badge" : "muted num"} style={over ? { background: "var(--col-dr-soft)", color: "var(--col-dr)" } : { fontSize: 11.5, fontWeight: 700 }}>
          ...
          <table className="table">
            ...
            <tbody>
              {[...NASABAH].sort((a, b) => b.dpd - a.dpd).slice(0, 30).map(n => {
                const p = petugasById(assign[n.id]);
                return (
                  <tr key={n.id}>
                    <td>
                    ...
                    <td>
                      <div className="center gap-2"><Avatar inisial={p.inisial} hue={p.hue} size={26} /><span style={{ fontSize: 12.5, fontWeight: 600 }}>{p.nama}</span></div>
                    </td>
      ...
      {moving && (
        <Modal onClose={() => setMoving(null)} max={420}>
          ...
          <div className="modal-body" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {PETUGAS.map(p => {
              const isCurrent = assign[moving.id] === p.id;
              const load = loadOf(p.id).length;
              return (
                <button key={p.id} onClick={() => !isCurrent && doAssign(moving.id, p.id)} disabled={isCurrent}
                  className="between" style={{ width: "100%", textAlign: "left", padding: 12, borderRadius: 12,
                    border: isCurrent ? "1.5px solid var(--accent)" : "1px solid var(--line)",
                    background: isCurrent ? "var(--accent-soft)" : "var(--surface)", cursor: isCurrent ? "default" : "pointer" }}>
                  <div className="center gap-3">
                  ...
                  {isCurrent ? <Badge c="var(--accent)" soft="transparent" icon={Ic.check}>Saat ini</Badge>
      ...
      {toast && (
        <div className="fade-up" style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", zIndex: 80,
          background: "var(--ink)", color: "white", padding: "12px 18px", borderRadius: 12, fontWeight: 700, fontSize: 13.5,
...
window.ScreenDistribusi = ScreenDistribusi;
