/* ============================================================
   SCREEN: Aplikasi Mobile Petugas Lapangan
   ============================================================ */

const ME = PETUGAS[0]; // Andi Pratama
const MY_TASKS = NASABAH.filter(n => n.petugas === ME.id).slice(0, 6);

function ScreenMobile() {
  const [tab, setTab] = useState("beranda");
  const [reportFor, setReportFor] = useState(null);
  const [done, setDone] = useState([]); // ids reported this session
        ...
        <IOSDevice width={372} height={806}>
          <div style={{ fontFamily: "var(--font)", height: "100%", display: "flex", flexDirection: "column", background: "var(--bg)", color: "var(--ink)" }}>
            <div style={{ flex: 1, overflowY: "auto", paddingTop: 54 }}>
              {!reportFor && tab === "beranda" && <MBeranda setTab={setTab} onReport={setReportFor} done={done} />}
              {!reportFor && tab === "rute" && <MRute onReport={setReportFor} done={done} />}
              {!reportFor && tab === "riwayat" && <MRiwayat done={done} />}
              {reportFor && <MLapor n={reportFor} onClose={() => setReportFor(null)} onDone={(id) => { setDone(d => [...d, id]); setReportFor(null); setTab("riwayat"); }} />}
            </div>
            {!reportFor && <MTabBar tab={tab} setTab={setTab} onReport={() => setReportFor(MY_TASKS.find(t => !done.includes(t.id)) || MY_TASKS[0])} />}
          </div>
        </IOSDevice>
      </div>
...
function MBeranda({ onReport, done }) {
  ...
  return (
    <div>
      ...
      {/* hero target card */}
      <div style={{ margin: "16px 16px 0", borderRadius: 22, padding: 18, color: "white", position: "relative", overflow: "hidden",
        background: "linear-gradient(145deg, var(--accent), var(--accent-700))", boxShadow: "0 10px 26px oklch(0.50 0.12 162 / 0.32)" }}>
        ...
        {MY_TASKS.map((n, i) => {
          ...
          return (
            <button key={n.id} onClick={() => !isDone && onReport(n)} disabled={isDone}
              style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 16, padding: 13,
                display: "flex", alignItems: "center", gap: 12, textAlign: "left", opacity: isDone ? 0.55 : 1 }}>
              <div style={{ width: 30, height: 30, borderRadius: 99, flex: "none", display: "grid", placeItems: "center",
                background: isDone ? "var(--accent)" : "var(--surface-2)", color: isDone ? "white" : "var(--ink-3)", fontWeight: 800, fontSize: 13 }} className="num">
                {isDone ? <Ic.check size={16} /> : i + 1}
              </div>
...
function MiniStatW({ label, value }) {
  return (
    <div style={{ flex: 1 }}>
    ...
    <div>
      ...
      <div style={{ margin: "0 16px 16px", borderRadius: 18, overflow: "hidden", border: "1px solid var(--line)", height: 160, position: "relative" }}>
        <svg viewBox="0 0 340 160" width="100%" height="100%" preserveAspectRatio="xMidYMid slice">
          ...
          {[[40,130],[100,90],[150,110],[210,60],[270,80]].map((p,i)=>(
            <g key={i}><circle cx={p[0]} cy={p[1]} r="9" fill="var(--surface)" stroke="var(--accent)" strokeWidth="2.5" />
            <text x={p[0]} y={p[1]+3.5} textAnchor="middle" fontSize="9" fontWeight="800" fill="var(--accent)">{i+1}</text></g>
          ))}
        ...
        {MY_TASKS.map((n, i) => (
          <div key={n.id} style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 16, padding: 13, display: "flex", gap: 12, alignItems: "center" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
              <span style={{ width: 24, height: 24, borderRadius: 99, background: "var(--accent-soft)", color: "var(--accent-ink)", display: "grid", placeItems: "center", fontWeight: 800, fontSize: 11 }} className="num">{i+1}</span>
    ...
    <div>
      ...
      {reported.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 30px", color: "var(--ink-4)" }}>
          <div className="stat-ic" style={{ width: 56, height: 56, margin: "0 auto 14px", background: "var(--surface-2)", color: "var(--ink-4)" }}><Ic.clipboard size={26} /></div>
          ...
          {reported.map(n => (
            <div key={n.id} style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 16, overflow: "hidden" }}>
              ...
              <div style={{ padding: 12 }}>
                <div className="between"><div style={{ fontWeight: 700, fontSize: 14 }}>{n.nama}</div><Badge c="var(--accent)" soft="var(--accent-soft)" icon={Ic.checkCircle}>Terkirim</Badge></div>
                <div className="muted center gap-2" style={{ fontSize: 12, marginTop: 4 }}><Ic.location size={12} style={{ color: "var(--accent)" }} />Lokasi tervalidasi · baru saja</div>
              </div>
            </div>
          ))}
        </div>
...
function MLapor({ n, onClose, onDone }) {
  const [hasil, setHasil] = useState("bayar");
  ...
  return (
    <div style={{ minHeight: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, padding: "0 16px 16px", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div>
          ...
          <div className="grid gap-2" style={{ gridTemplateColumns: "repeat(3,1fr)" }}>
            {[0,1,2].map(i => (
              foto > i
                ? <ImgPh key={i} label="✓ terfoto" h={76} style={{ color: "var(--accent)", background: "var(--accent-soft)", borderColor: "var(--accent)" }} />
                : <button key={i} onClick={() => setFoto(f => Math.max(f, i+1))} style={{ height: 76, borderRadius: 12, border: "1.5px dashed var(--line-2)", background: "var(--surface-2)", color: "var(--ink-4)", display: "grid", placeItems: "center" }}>
                    <Ic.camera size={20} />
                  </button>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="grid gap-2" style={{ gridTemplateColumns: "1fr 1fr" }}>
            {Object.entries(HASIL_KUNJUNGAN).map(([k, v]) => (
              <button key={k} onClick={() => setHasil(k)} style={{ padding: "11px 10px", borderRadius: 12, fontWeight: 700, fontSize: 12.5,
                border: hasil === k ? `1.5px solid ${v.c}` : "1px solid var(--line)", background: hasil === k ? v.soft : "var(--surface)", color: hasil === k ? v.c : "var(--ink-2)" }}>
                {v.label}
              </button>
            ))}
          </div>
        ...
        {(hasil === "bayar") && (
          <div>
            ...
            <div className="search" style={{ background: "var(--surface)" }}>
              ...
              <input value={Number(nominal).toLocaleString("id-ID")} onChange={e => setNominal(e.target.value.replace(/\D/g,""))} inputMode="numeric" style={{ fontWeight: 700 }} />
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <textarea className="input" rows={3} placeholder="Kondisi usaha, kesepakatan, dll…" value={catatan} onChange={e => setCatatan(e.target.value)} style={{ resize: "none", background: "var(--surface)" }} />
        </div>
      ...
      <div style={{ padding: 16, borderTop: "1px solid var(--line)", background: "var(--surface)" }}>
        <button onClick={submit} disabled={sending || foto === 0} className="btn btn-primary" style={{ width: "100%", padding: 14, fontSize: 15, opacity: foto === 0 ? 0.5 : 1 }}>
...
function MTabBar({ tab, setTab, onReport }) {
  const tabs = [
  ...
  return (
    <div style={{ borderTop: "1px solid var(--line)", background: "color-mix(in oklch, var(--surface) 90%, transparent)", backdropFilter: "blur(10px)",
      display: "flex", padding: "8px 8px 26px", alignItems: "center" }}>
      {tabs.map(t => {
        if (t.k === "_add") return (
          <div key="add" style={{ flex: 1, display: "grid", placeItems: "center" }}>
            <button onClick={onReport} style={{ width: 50, height: 50, borderRadius: 99, border: "none", background: "var(--accent)", color: "white",
        ...
        const on = tab === t.k;
        return (
          <button key={t.k} onClick={() => setTab(t.k)} style={{ flex: 1, background: "none", border: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 3,
