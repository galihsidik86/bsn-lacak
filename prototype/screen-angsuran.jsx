/* ============================================================
   SCREEN: Pergerakan Angsuran — arus pembayaran + ledger transaksi
   ============================================================ */

// build a transaction ledger from data
const TRX = (() => {
  const metode = [
  ...
  for (let i = 0; i < 22; i++) {
    ...
    out.push({ id: "TRX" + (id + i), nasabah: n, petugas: p, metode: m, jam,
      nominal: n.angsuran * (1 + (i % 3 === 0 ? 1 : 0)), status: i % 9 === 0 ? "pending" : "berhasil" });
  }
  return out.sort((a, b) => b.jam.localeCompare(a.jam));
})();

function ScreenAngsuran() {
  const [range, setRange] = useState("14h");
  const totalHari = TRX.filter(t => t.status === "berhasil").reduce((s, t) => s + t.nominal, 0);
  const byMethod = ["tunai","transfer","autodebet"].map(k => {
    const items = TRX.filter(t => t.metode.k === k && t.status === "berhasil");
    return { k, label: items[0]?.metode.label || k, value: items.reduce((s, t) => s + t.nominal, 0), n: items.length,
  ...
  return (
    <div className="content">
      <div className="stat-grid fade-up" style={{ gridTemplateColumns: "repeat(4,1fr)", marginBottom: 20 }}>
        ...
        <Stat icon={Ic.check} label="Transaksi Berhasil" value={TRX.filter(t=>t.status==="berhasil").length} sub={`${TRX.filter(t=>t.status==="pending").length} pending`} tint="var(--sms)" soft="oklch(0.93 0.04 245)" />
        <Stat icon={Ic.target} label="Pencapaian Target" value="78%" delta="6%" deltaDir="up" sub="target bulanan" tint="var(--col-dpk)" soft="var(--col-dpk-soft)" />
        <Stat icon={Ic.trend} label="Rata-rata / Transaksi" value={RPjt(Math.round(totalHari / TRX.filter(t=>t.status==="berhasil").length))} sub="nilai angsuran" />
      </div>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "1.5fr 1fr", marginBottom: 16 }}>
        <div className="card card-pad fade-up">
          <div className="between" style={{ marginBottom: 14 }}>
            ...
            <div className="seg">
              {["7h","14h","30h"].map(r => (
                <button key={r} className={range === r ? "on" : ""} onClick={() => setRange(r)}>{r === "7h" ? "7 hari" : r === "14h" ? "14 hari" : "30 hari"}</button>
              ))}
            </div>
          </div>
              ...
              {byMethod.map(m => (
                <div key={m.k}>
                  <div className="between" style={{ marginBottom: 4 }}>
                    <span className="center gap-2" style={{ fontWeight: 700, fontSize: 12.5 }}><span className="dot" style={{ background: m.color }} />{m.label}</span>
            ...
            <tbody>
              {TRX.map(t => (
                <tr key={t.id}>
                  ...
                  <td><div className="center gap-2"><Avatar inisial={t.petugas.inisial} hue={t.petugas.hue} size={24} /><span style={{ fontSize: 12.5, fontWeight: 600 }}>{t.petugas.nama.split(" ")[0]}</span></div></td>
                  <td><span className="badge" style={{ background: "var(--surface-2)", color: t.metode.c }}><span className="dot" style={{ background: t.metode.c }} />{t.metode.label}</span></td>
...
window.ScreenAngsuran = ScreenAngsuran;
