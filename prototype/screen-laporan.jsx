/* ============================================================
   SCREEN: Laporan Kunjungan Harian (dengan foto)
   ============================================================ */

function ScreenLaporan() {
  const [fPet, setFPet] = useState("all");
  const [fHasil, setFHasil] = useState("all");
  ...
  const counts = Object.keys(HASIL_KUNJUNGAN).map(h => ({ h, n: KUNJUNGAN.filter(k => k.hasil === h).length }));
    ...
    <div className="content">
      ...
      <div className="stat-grid fade-up" style={{ gridTemplateColumns: "repeat(4,1fr)", marginBottom: 20 }}>
        ...
        {counts.slice(0, 3).map(({ h, n }) => {
          ...
          return <Stat key={h} icon={h === "bayar" ? Ic.wallet : h === "janji" ? Ic.clock : Ic.user}
            label={hk.label} value={n} tint={hk.c} soft={hk.soft} sub={Math.round(n/KUNJUNGAN.length*100)+"% dari kunjungan"} />;
        })}
      </div>
          ...
          <div className="center gap-3" style={{ flexWrap: "wrap" }}>
            ...
            <div className="seg">
              ...
              {Object.entries(HASIL_KUNJUNGAN).map(([k, v]) => (
                <button key={k} className={fHasil === k ? "on" : ""} onClick={() => setFHasil(k)}>{v.label.split("/")[0].split(" ")[0]}</button>
              ))}
            </div>
          </div>
          ...
          return (
            <button key={k.id} onClick={() => setSel(k)} className="card fade-up"
              ...
              <div style={{ position: "relative" }}>
                <ImgPh label={`◦ FOTO KUNJUNGAN ◦\n${n.nama}`} h={150} style={{ borderRadius: 0, border: "none", borderBottom: "1px solid var(--line)", whiteSpace: "pre-line" }} />
              ...
              <div style={{ padding: 14, flex: 1, display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ fontSize: 12.5, color: "var(--ink-2)", marginTop: 8, lineHeight: 1.5, flex: 1,
                  display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{k.catatan}</div>
                <div className="between" style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid var(--line)" }}>
                  <div className="center gap-2"><Avatar inisial={p.inisial} hue={p.hue} size={24} /><span style={{ fontSize: 12, fontWeight: 600 }}>{p.nama.split(" ")[0]}</span></div>
                  <span className="num muted center gap-2" style={{ fontSize: 11.5, fontWeight: 600 }}><Ic.clock size={12} />{k.jam}</span>
                </div>
              </div>
            </button>
          );
...
function LaporanDetail({ k, onClose }) {
  const p = petugasById(k.petugas), n = nasabahById(k.nasabah), h = HASIL_KUNJUNGAN[k.hasil];
  return (
    <Modal onClose={onClose} max={620}>
      <div className="modal-head">
        <div style={{ flex: 1 }}>
          <div className="center gap-2"><span className="section-title">Laporan Kunjungan</span><span className="badge" style={{ background: h.soft, color: h.c }}>{h.label}</span></div>
      ...
      <div className="modal-body">
        ...
        <div className="card card-pad center gap-3" style={{ marginTop: 16, boxShadow: "none", background: "var(--surface-2)", padding: "12px 14px" }}>
          ...
          {k.valid ? <Badge c="var(--accent)" soft="var(--accent-soft)" icon={Ic.checkCircle}>Tervalidasi</Badge>
...
window.ScreenLaporan = ScreenLaporan;
